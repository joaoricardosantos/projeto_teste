...
# IMPORTANTE: rotas estáticas (/templates, /templates/active)
# devem ser declaradas ANTES das rotas com parâmetro dinâmico
# (/templates/{template_id}) para evitar conflito de roteamento.
# ─────────────────────────────────────────────
...
def create_template(request, payload: TemplateIn):
    ...
    Se is_active=True, desativa todos os outros antes de salvar.
    Retorna 200 (em vez de 201) para simplificar o tratamento no frontend.
    """
    if not request.auth.is_staff and not request.auth.is_superuser:
        raise HttpError(403, "Admin_privileges_required")
...
def get_active_template(request):
    ...
    Rota estática — deve ficar ANTES de /templates/{template_id}.
    """
    try:
